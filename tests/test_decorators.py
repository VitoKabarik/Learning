...
def test_log(capsys):
    with tempfile.NamedTemporaryFile(mode='a+t', delete=False) as logfile:
    ...
    @log()
    def search_key(dictionary, the_key):
    ...
    @log()
    def concatenation(str1, str2, str3, str4):
    ...
    assert captured.out == f"{now} concatenation TypeError: can only concatenate str (not \"bool\") to str. Inputs: (), {assertion_kwargs}\n"
