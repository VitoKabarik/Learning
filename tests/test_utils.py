...
def test_show_transactions(mock_file):
    assert show_transactions('some_path.json') == {'id': 41428829, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364', 'operationAmount': {'amount': '8221.37', 'currency': {'name': 'USD', 'code': 'USD'}}, 'description': 'Перевод организации', 'from': 'MasterCard 7158300734726758', 'to': 'Счет 35383033474447895560'}
    mock_file.assert_called_once_with('some_path.json', encoding='utf-8')


@patch('json.load')
@patch('builtins.open')
@pytest.mark.parametrize("exp_err, exp_output", [
    (FileNotFoundError(), []),
    (ValueError(), []),
    (json.JSONDecodeError('msg', 'doc', 0), [])
])
def test_show_transactions_with_wrong(mock_open_obj, mock_json, exp_err, exp_output):
